	...
	// Проще не удалять iframe, а изменять его src, но тогда в чате кнопки Вперед и Назад будут перезагружать чат.
	{
		...
		if (чСостояниеЧата === ЧАТ_СКРЫТ || чСостояниеЧата === ЧАТ_ПАНЕЛЬ)
		{
			if (_узЧат === null)
			{
				...
				_узЧат.id = 'чат';
				// UNDONE https://bugs.chromium.org/p/chromium/issues/detail?id=793280
...
const м_Управление = (() =>
{
	...
	function ОбработатьДвижениеМыши(оСобытие)
	{
		try
		{
			...
			document.removeEventListener('mousemove', ОбработатьДвижениеМыши);
	...
	function ПерехватитьДвижениеМыши()
	{
		try
		{
			document.addEventListener('mousemove', ОбработатьДвижениеМыши);
		}
		catch (пИсключение)
		{
			м_Отладка.ПойманоИсключение(пИсключение);
	...
	function ОбработатьЛевыйЩелчок(оСобытие)
	{
		try
		{
			...
			if (ОтключитьАвтоскрытие() || оСобытие.button !== ЛЕВАЯ_КНОПКА)
			{
				return;
			}
	...
	function ОбработатьПравыйЩелчок(оСобытие)
	{
		try
		{
			ОтключитьАвтоскрытие();
			if (оСобытие.target.id === 'глаз')
			{
				оСобытие.preventDefault();
			}
		}
	...
	// TODO Проверять <input type=text>.
	{
		try
		{
			...
			case 112: // F1. В Chrome 64 не перехватывается.
				if (лНажатие1)
				{
					м_Новости.ОткрытьСправку();
				}
	...
	function ОбработатьОткрытиеОкнаЧата(оСобытие)
	{
		try
		{
...
(() =>
{
	...
	function ЭтотКаналУжеОткрыт(сКодКанала)
	{
		...
		for (var оОкноРасширения of chrome.extension.getViews())
		{
			{
				if (оОкноРасширения === window || оОкноРасширения.closed || ВыделитьКанал(оОкноРасширения.location).сКодКанала !== сКодКанала)
				{
					continue;
				}
			}
			оОкноРасширения.chrome.tabs.getCurrent(function(оВкладка)
			{
				try
				{
