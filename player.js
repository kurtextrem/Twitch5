...
const ВЕРСИЯ_РАСШИРЕНИЯ = '2018.2.17';
...
function ЗаписатьТекстВЛокальныйФайл(сТекст, сТипДанных, сИмяФайла)
{
	...
	// click() не работает если ссылка не вставлена в документ.
...
ВводЧисла.prototype.ПослеИзменения = function() {};
...
const м_Отладка = (function()
{
	...
	function ПоказатьФорму(оДокумент, сИдентификаторФормы, лНастроитьФон)
	{
		...
		// HACK Edge 16: "живые" коллекции элементов (forms, children, getElementsByTagName)
		// из другого контекста (iframe) не перечисляются в for...of.
		for (let узПоказатьИлиСкрыть of оДокумент.querySelectorAll('form'))
		{
	...
	function ПоказатьИОтправитьОтчет(оОтчет, буфОтправить)
	{
		...
		.then(оДокумент =>
		{
			...
			оДокумент.body.addEventListener('reset', оСобытие =>
			{
			...
			оДокумент.body.addEventListener('submit', оСобытие =>
			{
				оСобытие.preventDefault();
	...
	function ПойманоИсключение(пИсключение)
	{
		ЗавершитьРаботуИОтправитьОтчет(
			пИсключение instanceof Error ?   `${пИсключение}\n${пИсключение.stack}`
...
const м_i18n = (function()
{
	function GetText(sName)
	{
		Проверить(typeof sName === 'string');
	...
	function TranslatePage(оДокумент)
	{
		м_Журнал.Вот('[i18n] Перевод страницы');
		for (let elTranslate of оДокумент.querySelectorAll('*[data-i18n]'))
		{
			const sNames = elTranslate.getAttribute('data-i18n');
			const sNamesDelimiter = sNames.indexOf('^');
			if (sNamesDelimiter !== 0)
			{
				// Commentary for AMO reviewers: HTML content is taken from the file messages.json. See GetText().
				elTranslate.innerHTML = GetText(sNamesDelimiter === -1 ? sNames : sNames.slice(0, sNamesDelimiter));
			}
			if (sNamesDelimiter !== -1)
			{
				elTranslate.title = GetText(sNames.slice(sNamesDelimiter + 1));
			}
		}
	}
	...
	function ПрошлоДнейПрописью(дСДаты)
	{
		...
		{
			ПрошлоДнейПрописью.мсДни = GetText('J0132').split('^');
...
const м_Тащилка = (function()
{
	...
	function ИзменитьПоложениеОкна(чШаг, узТащится, чИзменениеX, чИзменениеY)
	{
		...
		{
		case 1:
			var оСтиль = getComputedStyle(узТащится);
...
const м_Настройки = (function()
{
	...
	ВЕРСИЯ_НАСТРОЕК  Версия расширения  Изменения
	              ...
	              2         UNDONE      Добавлена проверка значений
	***/
	...
	const _оНастройки =
	{
		...
		чСостояниеЧата:                      Настройка.СоздатьПеречисление(ЧАТ_ЗАКРЫТ, [ЧАТ_ЗАКРЫТ, ЧАТ_СКРЫТ, ЧАТ_ПАНЕЛЬ]),
		...
		кЗаначка:                            Настройка.СоздатьДиапазон(0, 0, 2),
		                                     // Проценты.
	...
	// Возвращает true, если версия настроек не совместима с ВЕРСИЯ_НАСТРОЕК, большая часть настроек удалена,
	// и их также необходимо удалить из хранилища расширения.
	{
	...
	function Импорт(оИзФайла)
	{
		...
		оЧиталка.addEventListener('loadend', function()
		{
			try
			{
	...
	function ПолучитьДанныеДляОтчета()
	{
		...
		for (let сИмя of Object.keys(_оНастройки))
		{
			if (_оНастройки[сИмя].пТекущее !== _оНастройки[сИмя].пНачальное)
			{
...
const м_Чат = (function()
{
	...
	{
		switch (м_Настройки.Получить('чПоложениеПанелиЧата'))
		{
		case ПРАВАЯ_СТОРОНА:
			document.body.classList.add('чатсправа');
			document.body.classList.remove('чатвверху', 'чатвнизу', 'чатслева');
		...
		case НИЖНЯЯ_СТОРОНА:
			...
			document.body.classList.remove('чатвверху', 'чатсправа', 'чатслева');
			_узЧат.setAttribute('height', м_Настройки.Получить('чВысотаПанелиЧата'));
			_узЧат.removeAttribute('width');
			break;

		case ЛЕВАЯ_СТОРОНА:
			document.body.classList.add('чатслева');
			document.body.classList.remove('чатвверху', 'чатсправа', 'чатвнизу');
			_узЧат.setAttribute('width', м_Настройки.Получить('чШиринаПанелиЧата'));
			_узЧат.removeAttribute('height');
			break;

		case ВЕРХНЯЯ_СТОРОНА:
			document.body.classList.add('чатвверху');
			document.body.classList.remove('чатсправа', 'чатвнизу', 'чатслева');
			_узЧат.setAttribute('height', м_Настройки.Получить('чВысотаПанелиЧата'));
			_узЧат.removeAttribute('width');
			break;
	...
	function ПереключитьПоложениеПанели(оСобытие)
 	{
		...
		{
			оСобытие.preventDefault();
			var чПоложениеПанелиЧата = м_Настройки.Получить('чПоложениеПанелиЧата');
			switch (чПоложениеПанелиЧата)
			{
				case ПРАВАЯ_СТОРОНА:  чПоложениеПанелиЧата = НИЖНЯЯ_СТОРОНА;  break;
				case НИЖНЯЯ_СТОРОНА:  чПоложениеПанелиЧата = ЛЕВАЯ_СТОРОНА;   break;
				case ЛЕВАЯ_СТОРОНА:   чПоложениеПанелиЧата = ПРАВАЯ_СТОРОНА;  break;
				case ВЕРХНЯЯ_СТОРОНА: чПоложениеПанелиЧата = ПРАВАЯ_СТОРОНА;  break;
				default: Проверить(false);
			}
			м_Настройки.Изменить('чПоложениеПанелиЧата', чПоложениеПанелиЧата);
			ОбновитьПоложениеПанели();
		}
		catch (пИсключение)
		{
	...
	{
		const чПоложениеПанелиЧата = м_Настройки.Получить('чПоложениеПанелиЧата');
		const лИзменитьШирину = чПоложениеПанелиЧата === ПРАВАЯ_СТОРОНА || чПоложениеПанелиЧата === ЛЕВАЯ_СТОРОНА;
		const лОбратноеНаправление = чПоложениеПанелиЧата === ЛЕВАЯ_СТОРОНА || чПоложениеПанелиЧата === ВЕРХНЯЯ_СТОРОНА;
		switch (чШаг)
		{
		case 1:
			if (лИзменитьШирину)
...
const м_Новости = (function()
{
	...
	// Возвращает true, если добавлена хотя бы одна новость, включая справку.
	{
		...
		document.getElementById('новости-текст').textContent = '';
		...
		for (var мНовость of window.г_мНовости)
		{
			switch (мНовость[0])
			{
			case '2000.2.2': // Показывать всегда.
				AddOneNews(мНовость, 0);
				break;
			...
			default:
				...
				лДобавленыНовости = true;
				AddOneNews(мНовость, чВерсия);
			}
		}
		return лДобавленыНовости;
	}

	function AddOneNews(aNewsParts, nNewsTime)
	{
		const elInsertTo = document.getElementById('новости-текст');

		var sHtmlContent = '';
		for (var i = 2; i < aNewsParts.length; ++i)
		{
			sHtmlContent += м_i18n.GetText(aNewsParts[i]);
		}
		// Commentary for AMO reviewers: sHtmlContent is taken from the file messages.json. See м_i18n.GetText().
		elInsertTo.insertAdjacentHTML('afterbegin', sHtmlContent);

		const elHeader = document.createElement('h4');
		if (nNewsTime !== 0)
		{
			elHeader.textContent = `${(new Date(nNewsTime)).toLocaleDateString()}\u2002·\u2002${Текст(aNewsParts[1])}`;
		}
		else
		{
			elHeader.textContent = Текст(aNewsParts[1]);
		}
		elInsertTo.insertBefore(elHeader, elInsertTo.firstChild);

		elInsertTo.insertBefore(document.createElement('hr'), elHeader);
	}
...
(function()
{
	function ОткрытьМеню(узКнопкаИМеню)
	{
		узКнопкаИМеню.addEventListener('mouseleave', ОбработатьПокиданиеМеню);
		document.body.classList.remove('кнопкаименю-безанимации');
		узКнопкаИМеню.classList.add('открыто');
	}

	function ЗакрытьМеню(узКнопкаИМеню, лБезАнимации)
	{
		узКнопкаИМеню.removeEventListener('mouseleave', ОбработатьПокиданиеМеню);
		document.body.classList.toggle('кнопкаименю-безанимации', лБезАнимации);
		узКнопкаИМеню.classList.remove('открыто');
	}

	function ОбработатьПокиданиеМеню(оСобытие)
	{
		try
		{
	...
	function ОбработатьЩелчок(сСобытие, оСобытие)
	{
		...
		// Открыть или закрыть меню после нажатия на кнопку.
		if (узЩелчок.classList.contains('кнопкаименю-кнопка'))
		{
			const узКнопкаИМеню = узЩелчок.parentNode;
			if (узКнопкаИМеню.classList.contains('открыто'))
			{
				ЗакрытьМеню(узКнопкаИМеню, false);
			}
			else
			{
				ОткрытьМеню(узКнопкаИМеню);
			}
		}
		// Закрыть меню после щелчка по его пункту.
		else if (узЩелчок.classList.contains('пунктменю') && узЩелчок.hasAttribute('tabindex'))
		{
			const узКнопкаИМеню = узЩелчок.parentNode.parentNode;
			ЗакрытьМеню(узКнопкаИМеню, true);
		}
	}
...
const м_Управление = (function()
{
	...
	{
		var _sFullscreenElement = 'mozFullScreenElement';
		var _sRequestFullscreen = 'mozRequestFullScreen';
		var _sExitFullscreen    = 'mozCancelFullScreen';
		var _sFullscreenchange  = 'mozfullscreenchange';
	}
	...
	function ОбработатьЩелчок(оСобытие)
	{
		try
		{
			...
			const узЩелчок = оСобытие.target;
			const сПозывной = узЩелчок.id
			               || узЩелчок.name
			               || узЩелчок.parentNode.id
			               || узЩелчок.parentNode.name;
			...
			case 'одновременныхзагрузок':
				...
				м_Настройки.Изменить('кОдновременныхЗагрузок', Number.parseInt(узЩелчок.getAttribute('value'), 10));
				м_Статистика.ОчиститьИсторию();
				break;

			case 'заначка':
				Проверить(узЩелчок.checked);
				м_Настройки.Изменить('кЗаначка', Number.parseInt(узЩелчок.getAttribute('value'), 10));
				м_Статистика.ОчиститьИсторию();
				break;

			case 'менятьгромкостьколесом':
				Проверить(узЩелчок.checked);
				м_Настройки.Изменить('лМенятьГромкостьКолесом', !!Number.parseInt(узЩелчок.getAttribute('value'), 10));
				ЗапуститьИзменениеГромкостиКолесом();
				break;

			case 'анимацияинтерфейса':
				Проверить(узЩелчок.checked);
				м_Настройки.Изменить('лАнимацияИнтерфейса', !!Number.parseInt(узЩелчок.getAttribute('value'), 10));
	...
	function ОбновитьОкноНастроек()
	{
		...
		document.querySelector(`input[name="менятьгромкостьколесом"][value="${Number(м_Настройки.Получить('лМенятьГромкостьКолесом'))}"]`).checked = true;
		document.querySelector(`input[name="анимацияинтерфейса"][value="${Number(м_Настройки.Получить('лАнимацияИнтерфейса'))}"]`).checked = true;
	...
	function ОбработатьВыборФайлаДляИмпортаНастроек(оСобытие)
	{
		try
		{
...
const м_Проигрыватель = (function()
{
	...
	{
		// Если в настройках расширения Flashblock 1.5.20 запретить загрузку <video>, то
		// расширение удаляет <video> со страницы и меняет атрибут src на about:blank.
		if (!_oMediaElement.parentNode || (!_oMediaElement.src.startsWith('blob:') && !_oMediaElement.src.startsWith('mediasource:') && _oMediaElement.src !== ''))
		{
			м_Отладка.ЗавершитьРаботуИПоказатьСообщение('J0205');
		}
	...
	// Это единственная функция, которая во время прямой трансляции вызывает _oMediaElement.pause().
	{
		...
		// Не менять _oMediaElement.networkState на HTMLMediaElement.NETWORK_NO_SOURCE.
		// См. ПроверитьБлокировкуВоспроизведения().
	...
	function ДобавитьСледующийСегмент()
	{
		...
		.then(function()
		{
			...
			if (!_oMediaElement.seeking && !_oMediaElement.paused && !_oMediaElement.ended
			&& _oMediaElement.readyState >= HTMLMediaElement.HAVE_FUTURE_DATA)
			{
				var чНеПросмотрено = ПолучитьЗаполненностьБуфера().чНеПросмотрено;
				var чПорог = м_Статистика.ПолучитьДлительностьКадраВСекундах().чМаксимальная * 25 + 0.050;
		...
		.catch(function(пПричина)
		{
			try
			{
				...
				// Firefox 47: Исключение QuotaExceededError может быть кинуто из-за нехватки памяти (commit).
				// На данный момент других причин быть не должно.
				else if (ЭтоОбъект(пПричина) && пПричина.name === 'QuotaExceededError')
				{
					м_Отладка.ЗавершитьРаботуИПоказатьСообщение('J0200');
				}
				else
				{
	...
	function Запустить()
	{
		...
		{
			return ПроверитьБлокировкуВоспроизведения();
		}
		ПоказатьСостояние('Ой', '[Проигрыватель] Воспроизведение заблокировано до перехода на вкладку');
		м_События.ДобавитьОбработчик('фокусник-изменилосьсостояние', function ОбработатьПереходНаВкладку(сСобытие, оСостояние)
		{
			if (оСостояние.лПоказан)
			{
				м_События.УдалитьОбработчик(сСобытие, ОбработатьПереходНаВкладку);
				if (ПроверитьБлокировкуВоспроизведения())
				{
					ПоказатьСостояние('Ой', '[Проигрыватель] Воспроизведение разблокировано');
				}
				else
				{
					if (м_Управление.ПолучитьСостояние() !== СОСТОЯНИЕ_ОСТАНОВКА
	...
	function ПроверитьБлокировкуВоспроизведения()
	{
		Проверить(_oMediaElement.src === '' && _oMediaElement.networkState === HTMLMediaElement.NETWORK_EMPTY);
		_oMediaElement.play();
		if (_oMediaElement.paused)
		{
			ПоказатьСостояние('Ой', '[Проигрыватель] media.autoplay.enabled = false');
			Проверить(_oMediaElement.networkState === HTMLMediaElement.NETWORK_EMPTY);
			_oMediaElement.src = URL.createObjectURL(_oMediaSource);
			// HACK Ставим на паузу и ждем пока зритель не снимет проигрыватель с паузы.
...
const м_Загрузчик = (function()
{
	...
	// TODO Firefox 56: Некэшируемые файлы попадают в кэш в памяти, вытесняя из него полезные данные.
...
function Twitch(_сКодКанала)
{
	...
	// страницы, а также использует недокументированые возможности. Вреда может быть больше, чем пользы.
	{
		...
		if (_сИдТрансляции !== '' && чПозиция > 0)
		{
			...
			// HACK https://bugzilla.mozilla.org/show_bug.cgi?id=1344465
			// Также см. manifest.json, который запускает bug1344465.js.
			document.getElementById('создатьклип-форма').method = 'get';
			try
			{
				// Firefox 56: Если расширению не разрешено открывать всплывающие окна (например создание клипа запущено
				// нажатием комбинацией клавиш на клавиатуре), то будет кинуто XUL-исключение (и показано уведомление).
	...
	return {
		...
		ПолучитьАдресУбогогоПроигрывателя, ПолучитьАдресПанелиЧата,
...
function ПроизвольныйСписокВариантов(_сАбсолютныйАдресСпискаВариантов)
{
	...
	{
		return сАбсолютныйАдресСписка.replace(/^(file:\/\/\/.*)(\d+)(\.m3u8)$/, function(сСтрока, сЧасть1, сЧасть2, сЧасть3)
		{
			return сЧасть1 + (Number.parseInt(сЧасть2, 10) + 1) + сЧасть3;
		});
	}
	...
	return {
		ПолучитьАбсолютныйАдресСпискаВариантов, ПолучитьАбсолютныйАдресСпискаСегментов,
		ПолучитьАдресУбогогоПроигрывателя: НетАдреса, ПолучитьАдресПанелиЧата: НетАдреса,
...
function ЭтотКаналУжеОткрыт(сКодКанала)
{
	...
	// Firefox 53+: Проигрыватель почти всегда запускается как content script.
...
function НачатьРаботу()
{
	...
	// <video> используется в м_Управление и м_Проигрыватель. См. СледитьЗаОшибками().
	if (!document.getElementById('глаз') || document.getElementById('глаз').src !== '')
	{
		м_Отладка.ЗавершитьРаботуИПоказатьСообщение('J0205');
	}
