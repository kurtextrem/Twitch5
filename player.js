...
const м_Чат = (() =>
{
	...
	function ОбновитьЗатемнение()
	// Вызвать, если изменилась настройка лЗатемнитьЧат.
	{
		if (_узЧат)
		{
			м_Журнал.Окак('[Чат] Обновляю затемнение чата');
			// Нужно изменить URL чата.
			УдалитьПанель();
			ВставитьПанель();
		}
	}
	...
	function ИзменитьСостояниеЗакрытойПанели(чНовоеСостояние)
	{
	...
	return {
		...
		ИзменитьСостояниеЗакрытойПанели
	};
})();
...
const м_Новости = (() =>
{
	...
	{
		const elInsertTo = document.getElementById('текстновостей');

		if (elInsertTo.firstElementChild)
		{
			elInsertTo.insertBefore(document.createElement('hr'), elInsertTo.firstElementChild);
		}

		let sHtmlContent = '';
		for (let i = 2; i < aNewsParts.length; ++i)
		{
	...
	// Возвращает true, если добавлена хотя бы одна новость, включая справку.
	{
		...
		for (let мНовость of window.г_мНовости)
		{
			switch (мНовость[0])
			{
	...
	function ОбработатьПрокруткуТекста(оСобытие)
	{
		try
		{
...
const м_Управление = (() =>
{
	...
	function ОбработатьВключениеАвтоскрытия()
	{
		try
		{
	...
	function ОбработатьЛевыйЩелчок(оСобытие)
	{
		try
		{
			...
			const сПозывной = узЩелчок.id || узЩелчок.name || узЩелчок.parentNode.id || узЩелчок.parentNode.name;
			...
			case 'затемнитьчат':
				Проверить(узЩелчок.checked);
				м_Настройки.Изменить('лЗатемнитьЧат', !!Number.parseInt(узЩелчок.value, 10));
				м_Чат.ОбновитьЗатемнение();
				break;
			...
			case 'состояниезакрытогочата':
				Проверить(узЩелчок.checked);
				м_Чат.ИзменитьСостояниеЗакрытойПанели(Number.parseInt(узЩелчок.value, 10));
				break;
	...
	function ОбновитьОкноНастроек()
	{
		...
		document.querySelector(`input[name="затемнитьчат"][value="${Number(м_Настройки.Получить('лЗатемнитьЧат'))}"]`).checked = true;
	...
	function ПоказатьМетаданныеЗрителя(оМетаданные)
	{
		...
		if (оМетаданные.чПодписка !== undefined)
		{
			...
			else
			{
				...
				document.getElementById('зритель-уведомление').checked = оМетаданные.чПодписка === ПОДПИСКА_УВЕДОМЛЯТЬ;
			}
		}
		return true;
	}
