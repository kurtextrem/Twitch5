...
const РАЗДЕЛЯТЬ_ПО_КЛЮЧЕВЫМ_КАДРАМ = ВЕРСИЯ_ДВИЖКА_БРАУЗЕРА < 50;
...

{
	...
	***/
	...
	{
		if (РАЗДЕЛЯТЬ_ПО_КЛЮЧЕВЫМ_КАДРАМ)
		{
	...
	function ОбработатьСообщение()
	{
		...
		м_Журнал.Вот(`НАЧАЛО ПРЕОБРАЗОВАНИЯ СЕГМЕНТА ${_оИсходныйСегмент.чНомер} Размер=${(_оИсходныйСегмент.пДанные.byteLength / 1024 / 1024).toFixed(2)}мб Длительность=${_оИсходныйСегмент.чДлительность} Разрыв=${_лРазрыв}`);
