...
const ВЕРСИЯ_РАСШИРЕНИЯ = '2017.10.29';
...
const АВТОНАСТРОЙКА                   = Number.MIN_SAFE_INTEGER; // JSON не жрет NaN и Infinity.
...
function Текст(сКод)
{
	return м_и17я.ПолучитьТекст(сКод);
}
...
function СоздатьКопиюОбъекта(оИсходный)
{
	if (оИсходный === null)
	{
		return null;
	}
	Проверить(typeof оИсходный === 'object');
	var оКопия = Object.create(null);
	for (var сИмя of Object.keys(оИсходный))
	{
		var пЗначение = оИсходный[сИмя];
		Проверить(!ЭтоОбъект(пЗначение) && typeof пЗначение !== 'function');
		оКопия[сИмя] = пЗначение;
	}
	return оКопия;
}
...
function GetUrlHost(sAbsoluteUrl)
{
	return (new URL(sAbsoluteUrl)).host;
}

function ВставитьПосле(узВставить, узПосле)
{
	узПосле.parentNode.insertBefore(узВставить, узПосле.nextSibling);
}
...
ОтменаОбещания.ОБЕЩАНИЕ_ОТМЕНЕНО = new Error('AbortError');
...
function Ждать(ооОтменаОбещания, чМиллисекунды)
{
	...
	{
		var чТаймер = setTimeout(фВыполнить, чМиллисекунды);
		if (ооОтменаОбещания)
		{
			ооОтменаОбещания.ЗаменитьОбработчик(function()
			{
...
function ВводЧисла(чМинимум, чМаксимум, чШаг, чТочность, сАвтонастройка, пЗначение, сУзел)
{
	Проверить(чМинимум > АВТОНАСТРОЙКА && чМинимум <= чМаксимум && чТочность >= 0);
	Проверить(typeof сАвтонастройка === 'string');

	this._чМинимум = чМинимум;
	this._чМаксимум = чМаксимум;
	this._чШаг = чШаг;
	this._чТочность = чТочность;
	this._сАвтонастройка = сАвтонастройка;

	this._чТаймер = 0;
	this._кИнтервал = 0;
	this._чДобавить = 0;
	this._лПослеВвода = false;
	...
	{
		this._сИмяНастройки = пЗначение;
		this._ПрисвоитьЗначение(м_Настройки.Получить(пЗначение));
	}
	else if (!Number.isNaN(пЗначение))
	{
		this._ПрисвоитьЗначение(пЗначение);
	}
}

ВводЧисла.prototype._Показать = function()
{
	this._узУзел.children[1].value = this._чЗначение === АВТОНАСТРОЙКА ? Текст(this._сАвтонастройка) : м_и17я.ФорматироватьЧисло(this._чЗначение, this._чТочность);
};

ВводЧисла.prototype._ПрисвоитьЗначение = function(чНовоеЗначение)
// Возвращает true если новое значение отличается от старого.
{
	Проверить(ЭтоЧисло(чНовоеЗначение));
	if (чНовоеЗначение === АВТОНАСТРОЙКА)
	{
		Проверить(this._сАвтонастройка);
	}
	else
	{
		чНовоеЗначение = Ограничить(Округлить(чНовоеЗначение, this._чТочность), this._чМинимум, this._чМаксимум);
	}
	if (Number.isNaN(this._чЗначение))
	{
...
ВводЧисла.prototype.handleEvent = function(оСобытие)
{
	...
	try
	{
		...
		case 'mousedown':
			if (оСобытие.button === ЛЕВАЯ_КНОПКА && this._чТаймер === 0 && м_Фокусник.ПолучитьСостояние().лАктивен)
			{
				...
				this._чТаймер = setInterval(this._ОбработатьТаймер.bind(this), ИНТЕРВАЛ_ИЗМЕНЕНИЯ_ЗНАЧЕНИЯ);
				this._кИнтервал = 0;
				this._лПослеВвода = false;
...
ВводЧисла.prototype._ОбработатьТаймер = function()
{
	...
	try
	{
		...
		{
			if ((this._сАвтонастройка && this._чДобавить < 0 && this._чЗначение === this._чМинимум)
			||  (this._сАвтонастройка && this._чДобавить > 0 && this._чЗначение === this._чМаксимум))
			{
				var чНовоеЗначение = АВТОНАСТРОЙКА;
			}
			else if (this._чЗначение === АВТОНАСТРОЙКА && this._чДобавить > 0)
			{
				var чНовоеЗначение = this._чМинимум;
			}
			else if (this._чЗначение === АВТОНАСТРОЙКА && this._чДобавить < 0)
			{
				var чНовоеЗначение = this._чМаксимум;
			}
			else
			{
				var чНовоеЗначение = this._чЗначение + this._чДобавить;
			}
			if (this._ПрисвоитьЗначение(чНовоеЗначение))
			{
				this._лПослеВвода = true;
				this.ПослеВвода(this._чЗначение);
			}
		}
	}
	catch (пИсключение)
...
ВводЧисла.prototype._ЗавершитьИзменения = function()
{
	if (this._лПослеВвода)
	{
		if (this._сИмяНастройки)
		{
			м_Настройки.Изменить(this._сИмяНастройки, this._чЗначение);
		}
		this.ПослеИзменения(this._чЗначение);
	}
};

ВводЧисла.prototype.Обновить = function()
// Вызвать, если настройка, к которой привязан объект, была изменена.
// TODO Следить за изменениями настроек и обновлять объект автоматически?
{
	Проверить(this._сИмяНастройки);
	this._ПрисвоитьЗначение(м_Настройки.Получить(this._сИмяНастройки));
};

ВводЧисла.prototype.ПослеВвода = ВводЧисла.prototype.ПослеИзменения = function() {};

Object.defineProperty(ВводЧисла.prototype, 'чЗначение',
{
	get: function()
	{
	...
	set: function(чНовоеЗначение)
	{
		if (this._ПрисвоитьЗначение(чНовоеЗначение) && this._сИмяНастройки)
		{
			м_Настройки.Изменить(this._сИмяНастройки, this._чЗначение);
		}
	}
...
const м_Журнал = (function()
{
	...
	var _мсЖурнал = new Array(КОЛИЧЕСТВО_ЗАПИСЕЙ_В_ЖУРНАЛЕ); // Кольцевой буфер.
	...
	{
		return function(чЗначение)
		{
			return typeof чЗначение === 'number' ? чЗначение.toFixed(чТочность) : 'NaN';
		};
	}
...
const м_Отладка = (function()
{
	...
	function ПоказатьФорму(сИдентификаторФормы, лНастроитьФон)
	{
		if (лНастроитьФон)
		{
			м_Управление.ОтключитьПолноэкранныйРежим();
			// Заменить все классы, часть которых может мешать работе формы.
			document.body.className = сИдентификаторФормы;
		}

		for (var узПоказатьИлиСкрыть of document.body.children)
		{
			if (узПоказатьИлиСкрыть.id === сИдентификаторФормы)
			{
				узПоказатьИлиСкрыть.removeAttribute('hidden');
				
				var узФокус = узПоказатьИлиСкрыть.querySelector('*[autofocus]');
				if (узФокус)
				{
					узФокус.focus();
				}
			...
			{
				узПоказатьИлиСкрыть.setAttribute('hidden', '');
			}
		}
	}
	...
	{
		document.getElementById('отладка-текстсообщения').textContent = сСообщение;
		ПоказатьФорму('отладка-сообщение', true);
	}

	function ОтправитьОтчет(оОтчет, буфОтправить)
	{
		if (оОтчет.ПричинаЗавершенияРаботы === 'ОСТАВИТЬ ОТЗЫВ')
		{
			var узФорма = document.getElementById('отладка-отзыв');
		}
		else
		{
			var узФорма = document.getElementById('отладка-ошибка');
			ВставитьСсылкиДляСкачиванияФайлов(узФорма);
		}

		узФорма.querySelector('*[name="отладка-отчет"]').value = JSON.stringify(оОтчет);
		ПоказатьФорму(узФорма.id, true);

		document.getElementById('отладка-отменить').addEventListener('click', function()
		{
			window.location.reload();
		});

		var оЗапрос, оДанные;
		document.body.addEventListener('submit', function(оСобытие)
		{
			...
			case 'отладка-послеотправки':
				window.location.reload();
				break;

			default:
				document.getElementById('отладка-ходотправки').value = 0;
				ПоказатьФорму('отладка-идетотправка');
				...
				if (!оЗапрос)
				{
					...
					{
						document.getElementById('отладка-ходотправки').value = оСобытие.loaded / оСобытие.total;
					});
					оЗапрос.addEventListener('load', function()
					{
						ПоказатьФорму(this.status >= 200 && this.status <= 299 ? 'отладка-послеотправки' : 'отладка-сбойотправки');
					});
					оЗапрос.addEventListener('error', function()
					{
						ПоказатьФорму('отладка-сбойотправки');
					});
				...
				оЗапрос.open('POST', 'http://coolcmd.tk/tw5/report3.php');
				try
				{
					// HACK Firefox 44: Может кинуть исключение https://www.fxsitecompat.com/en-CA/docs/2016/network-error-for-async-xhr-now-fires-error-event-instead-of-throwing-getallresponseheaders-will-be-empty/
	...
	function СохранитьСписокВариантов(сСписокВариантов)
	{
		...
		{
			_сСписокВариантов = ОграничитьДлинуСтроки(сСписокВариантов, 5000);
		}
	}
	...
	function СохранитьСписокСегментов(сСписокСегментов)
	{
		if (!г_лРаботаЗавершена)
		{
			...
			_мСпискиСегментов.push(ОграничитьДлинуСтроки(сСписокСегментов, 2000));
		}
	}
	...
	function СоздатьОтчет(сПричинаЗавершенияРаботы, пОперативка, пПроцессор)
	{
	...
	function ЗавершитьРаботуИОтправитьОтчет(сПричинаЗавершенияРаботы, буфОтправить)
	{
		if (!г_лРаботаЗавершена)
		{
			...
			try
			{
				м_Проигрыватель.ПоказатьСостояние('Ой', '[Отладка] Завершаю работу');
				г_моОчередь.ПоказатьСостояние();
			}
			catch (и) {}
			ОбнюхатьОперативку(function(пОперативка)
			{
				ОбнюхатьПроцессор(function(пПроцессор)
				{
					ЗавершитьРаботу(false);
					ОтправитьОтчет(СоздатьОтчет(сПричинаЗавершенияРаботы, пОперативка, пПроцессор), буфОтправить);
				});
			});
		}
		throw undefined;
	...
	function ОставитьОтзыв()
	{
		try
		{
			ЗавершитьРаботуИОтправитьОтчет('ОСТАВИТЬ ОТЗЫВ');
		}
		catch (и) {}
	}
	...
	return {
		...
		ЗавершитьРаботуИПоказатьСообщение, ЗавершитьРаботуИОтправитьОтчет, ПойманоИсключение, ОставитьОтзыв
	};
})();

const м_Помойка = (function()
{
	const ВМЕСТИМОСТЬ_ПОМОЙКИ = 10000000;
	...
	function Сжечь()
	{
		if (_оПомойка)
		{
			м_Журнал.Вот(`[Помойка] Сжигаю ${_кбВПомойке} байтов`);
...
const м_и17я = (function()
{

	function ПолучитьТекст(сКод)
	{
		Проверить(typeof сКод === 'string');
		var сТекст = chrome.i18n.getMessage(сКод);
		if (!сТекст)
		{
			throw new Error(`Не найден текст ${сКод}`);
		}
		return сТекст;
	}

	function ПеревестиСтраницу()
	{
		м_Журнал.Вот('[i18n] Перевод страницы');
		for (var узПеревести of document.querySelectorAll('*[data-i18n]'))
		{
			узПеревести.innerHTML = ПолучитьТекст(узПеревести.firstChild.nodeValue.slice(1, -1)); // UNDONE AMO
		}
		for (var узПеревести of document.querySelectorAll('*[title^="&"]'))
		{
			узПеревести.title = ПолучитьТекст(узПеревести.title.slice(1, -1));
		}
	}
	...
	{
		var ч = Math.floor(кСекунды / 60 % 60);
		var с = Math.floor(кСекунды / 60 / 60) + (ч < 10 ? ' : 0' : ' : ') + ч;
		if (лНужныСекунды)
		{
	...
	function ПрошлоДнейПрописью(дСДаты)
	{
		...
		{
			ПрошлоДнейПрописью.мсДни = ПолучитьТекст('J0132').split('|');
...
const м_Фокусник = (function()
{
	...
	function ОбработатьСобытие(оСобытие)
	{
		try
		{
			if (г_лРаботаЗавершена)
...
const м_Тащилка = (function()
{
	const МИНИМАЛЬНЫЙ_ИНТЕРВАЛ_DRAGOVER = 50; // Миллисекунды.

	var _амОбработчики = new Map();

	var _узТащится = null;
	var _фОбработчик;
	var _чПоследнееDragOver;
	var _чМышьX1, _чМышьY1;
	var _чМышьX2, _чМышьY2;
	...
	function ИзменитьПоложениеОкна(чШаг, узТащится, чИзменениеX, чИзменениеY)
	{
		...
		{
		case 1:
			var оСтиль = window.getComputedStyle(узТащится);
	...
	function ОбработатьDragStart(оСобытие)
	{
		try
		{
			Проверить(_узТащится === null);
			_узТащится = оСобытие.target;
			_фОбработчик = _амОбработчики.get(_узТащится.id);
			...
			if (_фОбработчик)
			{
				...
				else
				{
					...
					_узТащится.classList.add('js-тащилка-скрыть');
					setImmediate(function()
					{
						// Переменная _узТащится уже может быть изменена.
						оСобытие.target.classList.remove('js-тащилка-скрыть');
					});
				}
			}
			else
	...
	function ОбработатьDragEnterИDrop(оСобытие)
	{
		try
		{
	...
	function ОбработатьDragOver(оСобытие)
	{
		try
		{
			// Firefox 44: Тащить могут кнопку из панели инструментов.
			if (_узТащится)
			{
				...
				var ч = performance.now();
				if (ч - _чПоследнееDragOver < МИНИМАЛЬНЫЙ_ИНТЕРВАЛ_DRAGOVER)
				{
					return;
				}
				_чПоследнееDragOver = ч;

				var лИзменилсяX = _чМышьX2 !== оСобытие.clientX;
				var лИзменилсяY = _чМышьY2 !== оСобытие.clientY;
				if (лИзменилсяX || лИзменилсяY)
				{
					_чМышьX2 = оСобытие.clientX;
					_чМышьY2 = оСобытие.clientY;
					_фОбработчик(2, _узТащится, _чМышьX1 - оСобытие.clientX, _чМышьY1 - оСобытие.clientY, лИзменилсяX, лИзменилсяY);
				}
			}
		}
		catch (пИсключение)
	...
	function ОбработатьDragEnd(оСобытие)
	{
		try
		{
			// Firefox 44: Тащить могут кнопку из панели инструментов.
			if (_узТащится)
			{
				м_Журнал.Окак(`[Тащилка] Заканчиваю тащить ${_узТащится.id} X=${_чМышьX2} Y=${_чМышьY2}`);
	...
	function ДобавитьОбработчик(сИдентификаторУзла, фОбработчик)
	{
		...
		var узУзел = document.getElementById(сИдентификаторУзла);
...
const м_Оформление = (function()
{
	...
	function ОбработатьВводЦвета(оСобытие)
	...
	function ОбработатьИзменениеЦвета(оСобытие)
	{
		try
		{
			if (оСобытие.target.matches(СЕЛЕКТОР_ВВОДА_ЦВЕТА))
	...
	{
		return сШаблон.replace(/{{.*?}}/g, function(сИмя)
		{
			var пЗначение = оПеременные[сИмя.slice(2, -2)];
			if (typeof пЗначение === 'string')
			{
				return пЗначение;
			}
	...
	function ОбновитьСтили()
	{
		...
		const чНепрозрачность = 1 - _оПрозрачность.чЗначение / 100;
		const оПеременные =
		{
	...
	function Запустить()
	{
		_оПрозрачность = new ВводЧисла(0, 80, 5, 0, '', 'чПрозрачность', 'прозрачность');
		_оПрозрачность.ПослеВвода = ОбновитьСтили;

		document.body.addEventListener('input', ОбработатьВводЦвета);
		document.body.addEventListener('change', ОбработатьИзменениеЦвета);
	...
	{
		return м_Загрузчик.ЗагрузитьТекст(null, chrome.extension.getURL('template.css'), 0, 'шаблон', false)
		.then(function(сРезультат)
		{
...
const м_Настройки = (function()
{
	const ВЕРСИЯ_НАСТРОЕК = 1;
	...
	const _мноОбязательныеНастройки = new Set(
	[
		'чСлучайноеЧисло',
		'сПредыдущаяВерсия',
		'чВерсияНастроек'
	...
	const _оНачальныеНастройки =
	{
		...
		сПредыдущаяВерсия:                       '2000.1.1',
		чВерсияНастроек:                         ВЕРСИЯ_НАСТРОЕК,
		чГромкость2:                             МАКСИМАЛЬНАЯ_ГРОМКОСТЬ / 2, // МИНИМАЛЬНАЯ_ГРОМКОСТЬ..МАКСИМАЛЬНАЯ_ГРОМКОСТЬ
		лПриглушить:                             false,
		лМенятьГромкостьКолесом:                 true,
		сНазваниеВарианта:                       'CoolCmd',
		чДлительностьПовтора2:                   60,                         // Секунды.
		чСостояниеЧата:                          0,                          // Константы ЧАТ_.
		чПоложениеПанелиЧата:                    ПРАВАЯ_СТОРОНА,             // Константы _СТОРОНА.
		чШиринаПанелиЧата:                       340,                        // CSS пикселы.
		чВысотаПанелиЧата:                       390,                        // CSS пикселы.
		//
		// Предустановки буферизации.
		//
		сПредустановкаВыбрана_буферизация:       'J0127',
		лПредустановкаЗаполнена_буферизация:     false,
		кОдновременныхЗагрузок:                  null,
		чНачалоВоспроизведения:                  null,                       // Секунды. 3,0 секунд ≈ 1..2 сегментов.
		                                                                     // TODO Измерять чНачалоВоспроизведения в сегментах?
		чРазмерБуфера:                           null,                       // Секунды. 8,5 секунд ≈ 3..5 сегментов.
		...
		чРастягиваниеБуфера:                     null,                       // Секунды.
		                                                                     // TODO Измерять чРастягиваниеБуфера в сегментах?
		кЗаначка:                                null,                       // Сегменты.
		                                                                     // TODO Заменить сегменты на время и ограничить заначку по времени.
		чИнтервалОпроса:                         null,                       // Проценты или АВТОНАСТРОЙКА.
		//
		// Предустановки оформления.
		//
		сПредустановкаВыбрана_оформление:        'J0122',
		лПредустановкаЗаполнена_оформление:      false,
		сЦветФона:                               null,
		сЦветГрадиента:                          '#ffffff',
		сЦветКнопок:                             null,
		сЦветЗаголовка:                          null,
	...
	var _оНастройки; // undefined чтобы не добавлять в JSON отчета об ошибке.

	// Не увеличивать слишком сильно, чтобы новое окно с проигрывателем не подхватило старые настройки.
	const ОТКЛАДЫВАТЬ_СОХРАНЕНИЕ_НА   = 500; // Миллисекунды.
	var _чТаймерОтложенногоСохранения = 0;
	var _оОтложенноеСохранение        = null;
	...
	function Сохранить(оСохранить, лОстальноеУдалить)
	{
		if (лОстальноеУдалить)
		{
			...
			м_Журнал.Вот('[Настройки] Все настройки удалены');
		}
		chrome.storage.local.set(оСохранить, ПроверитьРезультатСохранения);
		м_Журнал.Вот(`[Настройки] Настройки сохранены: ${м_Журнал.O(оСохранить)}`);
	}
	...
	function Восстановить()
	{
		...
		return new Promise(function(фВыполнить, фОтказаться)
		{
			chrome.storage.local.get(null, function(оВосстановленныеНастройки)
			{
				try
				{
					if (chrome.runtime.lastError)
	...
	// Меняет оСохранить.
	{
		...
		if (_чТаймерОтложенногоСохранения === 0)
		{
			...
			{
				м_Журнал.Окак(`[Настройки] Откладываю сохранение настроек на ${ОТКЛАДЫВАТЬ_СОХРАНЕНИЕ_НА}мс`);
	...
	function ЗавершитьОтложенноеСохранение()
	{
		try
		{
			м_Журнал.Окак('[Настройки] Завершаю отложенное сохранение');
	...
	function ПроверитьЗначениеНастройки(пЗначение)
	// XUL: Настройки хранятся в JSON.
	{
		Проверить(пЗначение === пЗначение && пЗначение !== Infinity && пЗначение !== -Infinity && пЗначение !== undefined && typeof пЗначение !== 'function' && typeof пЗначение !== 'symbol');
	}

	function ПроверитьВерсиюВосстановленныхНастроек(оВосстановленныеНастройки, оСохранить)
	{
		// Первый запуск расширения после установки?
		if (оВосстановленныеНастройки.сПредыдущаяВерсия === undefined)
		{
			return false;
		}

		// У ранних версий расширения не было чВерсияНастроек.
		const чВерсияНастроек = оВосстановленныеНастройки.чВерсияНастроек || 0;
		if (чВерсияНастроек === ВЕРСИЯ_НАСТРОЕК)
		{
			return false;
		}
		// Был сделан переход на расширение более ранней версии? Совместимость настроек в этом случае не поддерживается.
		if (чВерсияНастроек > ВЕРСИЯ_НАСТРОЕК)
		{
			for (var сИмя of Object.keys(оВосстановленныеНастройки))
			{
				delete оВосстановленныеНастройки[сИмя];
			}
			return true;
		}
		// Был сделан переход на расширение более поздней версии.
		оСохранить.чВерсияНастроек = оВосстановленныеНастройки.чВерсияНастроек = ВЕРСИЯ_НАСТРОЕК;
		
		if (чВерсияНастроек === 0)
		{
			// Версия 2017.7.3
			var чНовая = оВосстановленныеНастройки.чДлительностьПовтора2;
			var чСтарая = оВосстановленныеНастройки.чДлительностьПовтора;
			if (!ЭтоЧисло(чНовая) && ЭтоЧисло(чСтарая))
			{
				чНовая = чСтарая;
				if (чНовая < МИН_ДЛИТЕЛЬНОСТЬ_ПОВТОРА)
				{
					чНовая = МИН_ДЛИТЕЛЬНОСТЬ_ПОВТОРА;
				}
				if (чНовая > МАКС_ДЛИТЕЛЬНОСТЬ_ПОВТОРА)
				{
					чНовая = АВТОНАСТРОЙКА;
				}
				оСохранить.чДлительностьПовтора2 = оВосстановленныеНастройки.чДлительностьПовтора2 = чНовая;
			}

			var оМетаданные = _моМетаданныеПредустановок[0];
			Проверить(оМетаданные.амДанные === _амПредустановкиБуферизации);
			if ((оВосстановленныеНастройки.кОдновременныхЗагрузок !== undefined && оВосстановленныеНастройки.кОдновременныхЗагрузок !== 2)
			||  (оВосстановленныеНастройки.чНачалоВоспроизведения !== undefined && оВосстановленныеНастройки.чНачалоВоспроизведения !== 3)
			||  (оВосстановленныеНастройки.чРазмерБуфера          !== undefined && оВосстановленныеНастройки.чРазмерБуфера          !== 8.5)
			||  (оВосстановленныеНастройки.чРастягиваниеБуфера    !== undefined && оВосстановленныеНастройки.чРастягиваниеБуфера    !== 20)
			||  (оВосстановленныеНастройки.кЗаначка               !== undefined && оВосстановленныеНастройки.кЗаначка               !== 1)
			||  (оВосстановленныеНастройки.чИнтервалОпроса        !== undefined && оВосстановленныеНастройки.чИнтервалОпроса        !== АВТОНАСТРОЙКА))
			{
				оСохранить[оМетаданные.сВыбрана  ] = оВосстановленныеНастройки[оМетаданные.сВыбрана  ] = оМетаданные.сНастраиваемая;
				оСохранить[оМетаданные.сЗаполнена] = оВосстановленныеНастройки[оМетаданные.сЗаполнена] = true;
				const оПредустановка = оМетаданные.амДанные.get(_оНачальныеНастройки[оМетаданные.сВыбрана]);
				for (var сИмя of Object.keys(оПредустановка))
				{
					if (оВосстановленныеНастройки[сИмя] === undefined)
					{
						оСохранить[сИмя] = оВосстановленныеНастройки[сИмя] = оПредустановка[сИмя];
					}
				}
			}

			var оМетаданные = _моМетаданныеПредустановок[1];
			Проверить(оМетаданные.амДанные === _амПредустановкиОформления);
			if (оВосстановленныеНастройки.сИмяТемыОформления)
			{
				оСохранить[оМетаданные.сВыбрана] = оВосстановленныеНастройки[оМетаданные.сВыбрана] = оВосстановленныеНастройки.сИмяТемыОформления;
			}
			if (оВосстановленныеНастройки.оДанныеТемыОформления)
			{
				оСохранить[оМетаданные.сЗаполнена] = оВосстановленныеНастройки[оМетаданные.сЗаполнена] = true;
				оСохранить.сЦветФона      = оВосстановленныеНастройки.сЦветФона      = оВосстановленныеНастройки.оДанныеТемыОформления.сЦветФона;
				оСохранить.сЦветКнопок    = оВосстановленныеНастройки.сЦветКнопок    = оВосстановленныеНастройки.оДанныеТемыОформления.сЦветКнопок;
				оСохранить.сЦветЗаголовка = оВосстановленныеНастройки.сЦветЗаголовка = оВосстановленныеНастройки.оДанныеТемыОформления.сЦветЗаголовка;
				оСохранить.сЦветВыделения = оВосстановленныеНастройки.сЦветВыделения = оВосстановленныеНастройки.оДанныеТемыОформления.сЦветВыделения;
				оСохранить.чПрозрачность  = оВосстановленныеНастройки.чПрозрачность  = оВосстановленныеНастройки.оДанныеТемыОформления.чПрозрачность;
			}
		}
		return false;
	}
	...
	// Не переносим устаревшие настройки, добавляем новые, добавляем и сохраняем обязательные.
	{
		м_Журнал.Вот(`[Настройки] Настройки восстановлены: ${м_Журнал.O(оВосстановленныеНастройки)}`);
		Проверить(ЭтоОбъект(оВосстановленныеНастройки));
		const оСохранить = {};
		const лОстальноеУдалить = ПроверитьВерсиюВосстановленныхНастроек(оВосстановленныеНастройки, оСохранить);
		Проверить(!_оНастройки);
		_оНастройки = Object.create(null);
		for (var сИмя of Object.keys(_оНачальныеНастройки))
		{
			var пНачальноеЗначение = _оНачальныеНастройки[сИмя];
			ПроверитьЗначениеНастройки(пНачальноеЗначение);
			if (!оВосстановленныеНастройки.hasOwnProperty(сИмя))
			{
				if (_мноОбязательныеНастройки.has(сИмя))
				{
					оСохранить[сИмя] = пНачальноеЗначение;
				}
				_оНастройки[сИмя] = пНачальноеЗначение;
			}
			else
			{
				var пВосстановленноеЗначение = оВосстановленныеНастройки[сИмя];
				if (ЭтоОбъект(пНачальноеЗначение) && ЭтоОбъект(пВосстановленноеЗначение))
				{
					var оНастройки2 = Object.create(null);
					for (var сИмя2 of Object.keys(пНачальноеЗначение))
					{
						var пНачальноеЗначение2 = пНачальноеЗначение[сИмя2];
						ПроверитьЗначениеНастройки(пНачальноеЗначение2);
						// ЗавершитьВосстановление() и СоздатьКопиюОбъекта() не поддерживают вложенные объекты.
						Проверить(!ЭтоОбъект(пНачальноеЗначение2));
						if (!пВосстановленноеЗначение.hasOwnProperty(сИмя2))
						{
							оНастройки2[сИмя2] = пНачальноеЗначение2;
						}
						else
						{
							var пВосстановленноеЗначение2 = пВосстановленноеЗначение[сИмя2];
							Проверить(пНачальноеЗначение2 === null || пВосстановленноеЗначение2 === null || typeof пНачальноеЗначение2 === typeof пВосстановленноеЗначение2);
							оНастройки2[сИмя2] = пВосстановленноеЗначение2;
						}
					}
					_оНастройки[сИмя] = оНастройки2;
				}
				else
				{
					Проверить(пНачальноеЗначение === null || пВосстановленноеЗначение === null || typeof пНачальноеЗначение === typeof пВосстановленноеЗначение);
					_оНастройки[сИмя] = пВосстановленноеЗначение;
				}
			}
		}
	...
	function Сбросить()
	{
		...
		for (var сИмя of Object.keys(_оНачальныеНастройки))
		{
			if (_мноОбязательныеНастройки.has(сИмя))
			{
				оСохранить[сИмя] = _оНастройки[сИмя];
			}
			else
			{
				_оНастройки[сИмя] = _оНачальныеНастройки[сИмя];
			}
		}
	...
	function Получить2(сИмя)
	{
		...
		Проверить(сИмя in _оНастройки);

		for (var оМетаданные of _моМетаданныеПредустановок)
		{
			const оПредустановка = оМетаданные.амДанные.get(_оНастройки[оМетаданные.сВыбрана]);
			if (оПредустановка && (пЗначение = оПредустановка[сИмя]) !== undefined)
			{
				break;
			}
		}
		...
		{
			var пЗначение = _оНастройки[сИмя];
		}

		return ЭтоОбъект(пЗначение) ? СоздатьКопиюОбъекта(пЗначение) : пЗначение;
	}
	...
	function Изменить(сИмя, пЗначение)
	{
		Проверить(typeof сИмя === 'string');
		Проверить(сИмя in _оНастройки);
		ПроверитьЗначениеНастройки(пЗначение);
		Проверить(пЗначение === null || _оНастройки[сИмя] === null || typeof пЗначение === typeof _оНастройки[сИмя]);
		const оСохранить = {};
		...
		for (var оМетаданные of _моМетаданныеПредустановок)
		{
			...
			if (оПредустановка && оПредустановка.hasOwnProperty(сИмя))
			{
				...
				оСохранить[оМетаданные.сВыбрана  ] = _оНастройки[оМетаданные.сВыбрана  ] = оМетаданные.сНастраиваемая;
				оСохранить[оМетаданные.сЗаполнена] = _оНастройки[оМетаданные.сЗаполнена] = true;
				Object.assign(оСохранить,  оПредустановка);
				Object.assign(_оНастройки, оПредустановка);
		...
		{
			Проверить(пЗначение !== _оНастройки[сИмя]);
			пЗначение = СоздатьКопиюОбъекта(пЗначение);
		}
		if (пЗначение !== _оНастройки[сИмя])
		{
			оСохранить[сИмя] = _оНастройки[сИмя] = пЗначение;
		}
	...
	function ОбновитьСписокПредустановок(оМетаданные)
	{
		...
		узСписок.length = 0;
		const сВыбрать = _оНастройки[оМетаданные.сВыбрана];
		for (var сИмя of оМетаданные.амДанные.keys())
		{
			узСписок.add(new Option(Текст(сИмя), сИмя, сИмя === сВыбрать, сИмя === сВыбрать));
		}
		if (_оНастройки[оМетаданные.сЗаполнена])
		{
	...
	function ОбработатьИзменениеПредустановки(оСобытие)
	{
		try
		{
...
const м_Статистика = (function()
{
	...
	function ОбновитьСтатистику()
	{
		try
		{
			document.getElementById('статистика-длительностьпросмотра').textContent = м_и17я.ПеревестиСекундыВСтроку(performance.now() / 1000, true);
	...
	function СкрытьОкно()
	{
		if (ОкноПоказано())
		{
			clearInterval(_чТаймер); _чТаймер = 0;

			document.getElementById('статистика').setAttribute('hidden', '');
	...
	function ВыбранВариантТрансляции(сСобытие, мДанные)
	{
		...
		{
			document.getElementById('статистика-сервер').textContent = GetUrlHost(моСписокВариантов[чВыбранныйВариант].сАбсолютныйАдресСпискаСегментов);
		}
	}
	...
	function ДобавленыСегментыВОчередь(кСегментовДобавлено, кСекундДобавлено)
	{
		if (ОкноПоказано())
		{
			...
			_оСекундДобавлено.ДобавитьЧисло(кСекундДобавлено,
				...
				function(чЧисло)
				{
					return чЧисло / чВСреднемСегментовДобавлено < _nTargetDuration * ВЫДЕЛИТЬ_СЕКУНД_ДОБАВЛЕНО;
				}
			);
		}
	}
	...
	function ЗагруженСегмент(чРазмерСегмента, чДлительностьСегмента, чДлительностьЗагрузки, чОжиданиеОтвета)
	{
		if (ОкноПоказано())
		{
			...
			_оТолщинаКанала.ДобавитьЧисло(
				...
				function(чЧисло)
				{
					return чЧисло < чСредняяТолщинаСегмента;
				}
			);
	...
	return {
		ОкноПоказано, ПоказатьОкно, СкрытьОкно,
		ОбновитьЗначение,
		ОчиститьИсторию,
...
const м_Чат = (function()
{
	...
	var _узЧат = null; // iframe
	var _чНачальныйРазмер;


	function ОбработатьСообщение(оСобытие)
	{
		try
		{
			if (оСобытие.source === _узЧат.contentWindow
			&&  оСобытие.origin === м_Twitch.ПолучитьАдресПанелиЧата()[0]
			&&  оСобытие.data   === 'ВставитьСторонниеРасширения?')
			{
				м_Журнал.Вот('[Чат] Получен запрос на вставку сторонних расширений');
				// Оставляем обработчик message на случай перезагрузки iframe (из контекстного меню).
				chrome.management.getAll(function(моРасширения)
				{
					if (chrome.runtime.lastError)
					{
						м_Отладка.ЗавершитьРаботуИОтправитьОтчет(`Не удалось получить список расширений. ${chrome.runtime.lastError.message}`);
					}
					try
					{
						const оОтвет =
						{
							сЗапрос: оСобытие.data,
							сСторонниеРасширения: ''
						};
						for (var оРасширение of моРасширения)
						{
							if (оРасширение.enabled)
							{
								...
								case 'deofbbdfofnmppcjbhjibgodpcdchjii': /* Opera  */
									оОтвет.сСторонниеРасширения += 'BTTV ';
									break;
									
								case 'fadndhdgpmmaapbmfcknlfgcflmmmieb': /* Chrome */
								case 'djkpepcignmpfblhbfpmlhoindhndkdj': /* Opera  */
									оОтвет.сСторонниеРасширения += 'FFZ ';
									break;
									
								case 'aiimboljphncldaakcnapfolgnjonlea': /* Chrome */
									оОтвет.сСторонниеРасширения += 'FFZAP ';
									break;
								}
							}
						}
						м_Журнал.Вот(`[Чат] Послан ответ на вставку сторонних расширений: ${м_Журнал.O(оОтвет)}`);
						// Кидает исключение если iframe уже закрылся или сменил origin.
						оСобытие.source.postMessage(оОтвет, оСобытие.origin);
					}
					catch (пИсключение)
					{
						м_Журнал.Ой(`[Чат] Поймано исключение во время посылки ответа: ${пИсключение}`);
					}
				});
			}
		}
	...
	{
		if (м_Настройки.Получить('чПоложениеПанелиЧата') === ПРАВАЯ_СТОРОНА)
		{
			document.body.classList.add('чатсправа');
			document.body.classList.remove('чатвнизу');
			_узЧат.setAttribute('width', м_Настройки.Получить('чШиринаПанелиЧата'));
			_узЧат.removeAttribute('height');
		}
		else
		{
			document.body.classList.add('чатвнизу');
			document.body.classList.remove('чатсправа');
			_узЧат.setAttribute('height', м_Настройки.Получить('чВысотаПанелиЧата'));
			_узЧат.removeAttribute('width');
		}
	}
	...
	function ЗагрузитьЧат()
	{
		try
		{
			if (_узЧат && !_узЧат.src)
			{
				_узЧат.src = м_Twitch.ПолучитьАдресПанелиЧата().join('');
			}
		}
		catch (пИсключение)
		{
	...
	{
		var чСостояниеЧата = м_Настройки.Получить('чСостояниеЧата');
		м_Журнал.Окак(`[Чат] Новое состояние чата: ${чСостояниеЧата}`);
		document.body.classList.toggle('показатьчат', чСостояниеЧата === ЧАТ_ПАНЕЛЬ);
		if (чСостояниеЧата === ЧАТ_СКРЫТ || чСостояниеЧата === ЧАТ_ПАНЕЛЬ)
		{
			if (_узЧат === null)
			{
				window.addEventListener('message', ОбработатьСообщение);
				...
				_узЧат.id = 'чат';
				// UNDONE https://bugs.chromium.org/p/chromium/issues/detail?id=
				var чЗадержатьЗагрузку;
				if (ОбновитьСостояниеПанели.чТаймер === undefined
				&& (чЗадержатьЗагрузку = Math.round(1000 - performance.now())) > 0)
				{
					ОбновитьСостояниеПанели.чТаймер = setTimeout(ЗагрузитьЧат, чЗадержатьЗагрузку);
				}
				else
				{
					ЗагрузитьЧат();
				...
				ВставитьПосле(_узЧат, document.getElementById('размерчата'));
			}
		}
		else
		{
	...
	function ПереключитьПанель()
 	{
		м_Настройки.Изменить('чСостояниеЧата', м_Настройки.Получить('чСостояниеЧата') === ЧАТ_ПАНЕЛЬ ? ЧАТ_ЗАКРЫТ : ЧАТ_ПАНЕЛЬ);
		ОбновитьСостояниеПанели();
	}

	function СкрытьПанель()
	{
		м_Настройки.Изменить('чСостояниеЧата', ЧАТ_СКРЫТ);
		ОбновитьСостояниеПанели();
	}

	function ПереключитьПоложениеПанели(оСобытие)
 	{
		try
		{
			оСобытие.preventDefault();
			м_Настройки.Изменить('чПоложениеПанелиЧата', м_Настройки.Получить('чПоложениеПанелиЧата') === ПРАВАЯ_СТОРОНА ? НИЖНЯЯ_СТОРОНА : ПРАВАЯ_СТОРОНА);
			ОбновитьПоложениеПанели();
		}
		catch (пИсключение)
		{
	...
	function ИзменитьРазмерПанели(чШаг, узТащится, чИзменениеX, чИзменениеY, лИзменилсяX, лИзменилсяY)
	{
		switch (чШаг)
		{
		case 1:
			...
			{
				// Учитывать min-width.
				_чНачальныйРазмер = Number.parseInt(window.getComputedStyle(_узЧат).width, 10);
			}
			else
			{
				// Учитывать min-height.
				_чНачальныйРазмер = Number.parseInt(window.getComputedStyle(_узЧат).height, 10);
			}
		...
		case 2:
			if (м_Настройки.Получить('чПоложениеПанелиЧата') === ПРАВАЯ_СТОРОНА)
			{
				if (лИзменилсяX)
				{
					var а = Number.parseInt(window.getComputedStyle(document.getElementById('проигрывательичат')).width, 10);
					var б = Number.parseInt(window.getComputedStyle(document.getElementById('проигрыватель')).minWidth, 10);
					_узЧат.width = Math.max(Math.min(_чНачальныйРазмер + чИзменениеX, а - б), 0);
				}
			}
			else
			{
				if (лИзменилсяY)
				{
					var а = Number.parseInt(window.getComputedStyle(document.getElementById('проигрывательичат')).height, 10);
					var б = Number.parseInt(window.getComputedStyle(document.getElementById('проигрыватель')).minHeight, 10);
					_узЧат.height = Math.max(Math.min(_чНачальныйРазмер + чИзменениеY, а - б), 0);
				}
			}
			break;
		...
		case 3:
			if (м_Настройки.Получить('чПоложениеПанелиЧата') === ПРАВАЯ_СТОРОНА)
			{
				м_Настройки.Изменить('чШиринаПанелиЧата', Number.parseInt(window.getComputedStyle(_узЧат).width, 10));
			}
			else
			{
				м_Настройки.Изменить('чВысотаПанелиЧата', Number.parseInt(window.getComputedStyle(_узЧат).height, 10));
			}
			break;
	...
	{
		ОбновитьСостояниеПанели();
		м_Тащилка.ДобавитьОбработчик('размерчата', ИзменитьРазмерПанели)
		.addEventListener('contextmenu', ПереключитьПоложениеПанели);
	}
	
	return {
		Восстановить,
		ПереключитьПанель, СкрытьПанель,
		ОткрытьОкно, ЗакрытьОкно
	};
})();

const м_Новости = (function()
{
	...
	// Возвращает true, если добавлена хотя бы одна новость, включая справку.
	{
		...
		const узШаблон = document.getElementsByTagName('template')[0];
		while (узШаблон.nextElementSibling)
		{
			узШаблон.nextElementSibling.remove();
		}

		var лДобавленыНовости = false;
		var чДобавитьВерсию = typeof пДобавитьВерсию === 'number' ? пДобавитьВерсию : ПеревестиВерсиюВМиллисекунды(пДобавитьВерсию);
		// Edge 15: У DocumentFragment нет children.
		for (var узНовость of узШаблон.content.childNodes)
		{
			if (узНовость.nodeType === узНовость.ELEMENT_NODE)
			{
				const сВерсия = узНовость.getAttribute('data-версия');
				if (сВерсия === '2000.2.2')
				{
					// Показывать всегда.
					ДобавитьНовость(узНовость, 0, узШаблон);
				}
				else if (сВерсия === '2000.1.1')
				{
					// Справка.
					if (лДобавитьСправку)
					{
				...
				else
				{
					...
					лДобавленыНовости = true;
					ДобавитьНовость(узНовость, чВерсия, узШаблон);
				}
			}
		}
	...
	function ДобавитьНовость(узНовость, чДатаНовости, узШаблон)
	{
		узНовость = узНовость.cloneNode(true);
		const сКоды = узНовость.textContent.split(' ');
		Проверить(сКоды.length);
		узНовость.textContent = '';
		for (var сКод of сКоды)
		{
			узНовость.insertAdjacentHTML('beforeend', Текст(сКод.slice(1, -1)));
		}
		if (чДатаНовости !== 0)
		{
			const узЗаголовок = узНовость.firstElementChild;
			узЗаголовок.insertBefore(
				document.createTextNode((new Date(чДатаНовости)).toLocaleDateString() + (узЗаголовок.textContent.length !== 0 ? '\u2002·\u2002' : '')),
				узЗаголовок.firstChild
			);
		}
		ВставитьПосле(узНовость, узШаблон);
		ВставитьПосле(document.createElement('hr'), узШаблон);
	}

	function ПоказатьКнопку()
	{
		const узКнопка = document.getElementById('переключитьновости');
		узКнопка.removeAttribute('hidden');
		узКнопка.classList.add('непрочитано');
		м_События.ДобавитьОбработчик('окна-открытоокно', ОбработатьОткрытиеОкнаНовостей);
		м_События.ДобавитьОбработчик('окна-закрытоокно', ОбработатьЗакрытиеОкнаНовостей);
	}

	function ОбработатьОткрытиеОкнаНовостей(сСобытие, сИдОкна)
	{
		if (сИдОкна === 'новости')
		{
			document.getElementById('переключитьновости').classList.remove('непрочитано');
		}
	}

	function ОбработатьЗакрытиеОкнаНовостей(сСобытие, оСобытие)
	// Не удалять текст чтобы не портить анимацию закрытия.
	{
		if (оСобытие.сИдОкна === 'новости' && оСобытие.лКнопкой)
		{
			м_Настройки.Изменить('сПредыдущаяВерсия', ВЕРСИЯ_РАСШИРЕНИЯ);
			document.getElementById('переключитьновости').setAttribute('hidden', '');
			м_События.УдалитьОбработчик('окна-открытоокно', ОбработатьОткрытиеОкнаНовостей);
			м_События.УдалитьОбработчик('окна-закрытоокно', ОбработатьЗакрытиеОкнаНовостей);
		}
	}
	...
	function Запустить()
	{
		...
		if (сПредыдущаяВерсия !== ВЕРСИЯ_РАСШИРЕНИЯ)
		{
			...
			{
				var лДобавленыНовости = ДобавитьНовости(0, true);
			}
			else
			{
				var лДобавленыНовости = ДобавитьНовости(сПредыдущаяВерсия, false);
			}
			if (лДобавленыНовости)
			{
				ПоказатьКнопку();
			}
			else
	...
	function ОткрытьСправку()
	{
		...
		{
			if (document.getElementById('переключитьновости').hasAttribute('hidden'))
			{
				ДобавитьНовости(3, true, false);
			}
...
const м_Шкала = (function()
{
	...
	// TODO Несколько первых пикселов слева всегда перематывают на первый кадр?
	{
		try
		{
			if (м_Управление.ПолучитьСостояние() === СОСТОЯНИЕ_ПОВТОР)
			{
				var oRect = оСобытие.currentTarget.getBoundingClientRect();
				var nHoverLeft = Math.floor(oRect.left); // Chrome 52, Edge 15.
				var чПеремотатьДо = ОграничитьВремя((оСобытие.clientX - nHoverLeft) / (oRect.width - 1) * (_чКонец - _чНачало) + _чНачало);
...
const м_Окна = (function()
{
	function ПолучитьОткрытоеОкно()
	{
		const сИдОткрытогоОкна = document.body.getAttribute('data-открытоеокно');
		return сИдОткрытогоОкна ? сИдОткрытогоОкна : '';
	}

	function ЗакрытьОкно(сИдОкна, лКнопкой)
	{
		Проверить(typeof сИдОкна === 'string' && typeof лКнопкой === 'boolean');
		if (сИдОкна && сИдОкна === ПолучитьОткрытоеОкно())
		{
			document.getElementById(сИдОкна).classList.remove('открыто');
			document.body.removeAttribute('data-открытоеокно');
			м_События.ОбработатьСобытие('окна-закрытоокно', {сИдОкна, лКнопкой});
		}
	}
	...
	function ОткрытьОкно(сИдОкна)
	{
		...
		if (сИдОткрытогоОкна !== сИдОкна)
		{
			ЗакрытьОкно(сИдОткрытогоОкна, false);

			document.getElementById(сИдОкна).classList.add('открыто');
			document.body.setAttribute('data-открытоеокно', сИдОкна);
			м_События.ОбработатьСобытие('окна-открытоокно', сИдОкна);
		}
	}

	function ПереключитьОкно(сИдОкна)
	{
		Проверить(ЭтоНепустаяСтрока(сИдОкна));
		const сИдОткрытогоОкна = ПолучитьОткрытоеОкно();
		ЗакрытьОкно(сИдОткрытогоОкна, сИдОткрытогоОкна === сИдОкна);
		if (сИдОткрытогоОкна !== сИдОкна)
		{
			ОткрытьОкно(сИдОкна);
		}
	...
	function ОбработатьЩелчок(сСобытие, оСобытие)
	{
		...
		const сИдОкна = оСобытие.target.getAttribute('data-переключитьокно');
		ЗакрытьОкно(сИдОткрытогоОкна, сИдОткрытогоОкна === сИдОкна);

		if (сИдОкна && сИдОткрытогоОкна !== сИдОкна)
		{
			ОткрытьОкно(сИдОкна);
		}
	}
...
const м_Управление = (function()
{
	...
	const ИНТЕРВАЛ_АВТОСКРЫТИЯ           = 4000; // Миллисекунды.
	...
	function ОбработатьИзменениеПолноэкранногоРежима()
	{
		try
		{
			ОбновитьПолноэкранныйРежим();
	...
	function ВключитьПолноэкранныйРежим()
	{
		...
		_узПроигрыватель[_sRequestFullscreen]();
	}
	...
	function ОтключитьАвтоскрытие()
	{
		...
		{
			document.body.classList.remove('автоскрытие', 'безанимации');
		}
		else
		{
			clearTimeout(_чТаймерАвтоскрытия);
		}
		_чТаймерАвтоскрытия = setTimeout(ОбработатьТаймерАвтоскрытия, ИНТЕРВАЛ_АВТОСКРЫТИЯ);
	}
	...
	function ВключитьАвтоскрытие(лБезАнимации)
	{
		...
		if (_чТаймерАвтоскрытия !== 0)
		{
			...
			document.body.classList.toggle('безанимации', лБезАнимации);
			if (лБезАнимации)
			{
	...
	function ОбработатьТаймерАвтоскрытия()
	{
		try
		{
	...
	function ОбработатьДвижениеМыши(оСобытие)
	{
		try
		{
			...
			_узПроигрыватель.removeEventListener('mousemove', ОбработатьДвижениеМыши);
	...
	function ПерехватитьДвижениеМыши()
	{
		try
		{
			_узПроигрыватель.addEventListener('mousemove', ОбработатьДвижениеМыши);
		}
		catch (пИсключение)
		{
			м_Отладка.ПойманоИсключение(пИсключение);
	...
	function ЗапуститьАвтоскрытие()
	{
		...
		document.addEventListener('focusout', function(оСобытие)
		{
	...
	function ЗапуститьИзменениеГромкостиКолесом()
	{
		...
		{
			_узПроигрыватель.addEventListener('wheel', ОбработатьВращениеКолеса);
			_узПроигрыватель.addEventListener('mousedown', ОбработатьНажатиеМыши);
		}
		else
		{
			_узПроигрыватель.removeEventListener('wheel', ОбработатьВращениеКолеса);
			_узПроигрыватель.removeEventListener('mousedown', ОбработатьНажатиеМыши);
		}
	}

	function ОбработатьВращениеКолеса(оСобытие)
	{
		try
		{
	...
	function ОбработатьНажатиеМыши(оСобытие)
	{
		try
		{
	...
	{
		м_Журнал.Окак('[Управление] Сбрасываю настройки');
	...
	function ПереключитьОкноСтатистики()
	{
		...
		{
			м_Статистика.СкрытьОкно();
		}
		else
		{
			м_Статистика.ПоказатьОкно();
		}
	}
	...
	function ИзменитьПодпискуЗрителяНаКанал(чПодписка)
	{
		...
		if (ПоказатьМетаданныеЗрителя({чПодписка: ПОДПИСКА_ОБНОВЛЯЕТСЯ}))
		{
	...
	function ОбработатьЩелчок(оСобытие)
	{
		try
		{
			...
			const сПозывной = оСобытие.target.id
			...
			case 'одновременныхзагрузок':
				...
				м_Настройки.Изменить('кОдновременныхЗагрузок', Number.parseInt(оСобытие.target.getAttribute('value'), 10));
				м_Статистика.ОчиститьИсторию();
				break;

			case 'заначка':
				Проверить(оСобытие.target.checked);
				м_Настройки.Изменить('кЗаначка', Number.parseInt(оСобытие.target.getAttribute('value'), 10));
				м_Статистика.ОчиститьИсторию();
				break;

			case 'менятьгромкостьколесом':
				Проверить(оСобытие.target.checked);
				м_Настройки.Изменить('лМенятьГромкостьКолесом', !!Number.parseInt(оСобытие.target.getAttribute('value'), 10));
			...
			case 'оставитьотзыв':
				м_Отладка.ОставитьОтзыв();
				break;

			case 'поддержать':
				window.open('http://coolcmd.tk/tw5/donate.html');
				break;
			...
			case 'типтрансляции':
				Проверить(оСобытие.target.nodeName === 'BUTTON');
				if (!оСобытие.target.disabled)
				{
					location.assign(оСобытие.target.getAttribute('data-адрес'));
				}
				break;
			...
			case 'зритель-неуведомлять':
				ИзменитьПодпискуЗрителяНаКанал(ПОДПИСКА_НЕУВЕДОМЛЯТЬ);
				break;
			}
		}
		catch (пИсключение)
	...
	function ОбработатьНажатиеИОтпусканиеКлавы(оСобытие)
	...
	// TODO Проверять <input type=text>.
	{
		try
		{
			...
			var лНажатие = оСобытие.type === 'keydown';
			var лНажатие1 = лНажатие && !оСобытие.repeat;
	...
	{
		var уз;
		if (уз = document.querySelector(`input[name="одновременныхзагрузок"][value="${м_Настройки.Получить('кОдновременныхЗагрузок')}"]`))
		{
			уз.checked = true;
		}

		if (уз = document.querySelector(`input[name="заначка"][value="${м_Настройки.Получить('кЗаначка')}"]`))
		{
			уз.checked = true;
		}

		if (уз = document.querySelector(`input[name="менятьгромкостьколесом"][value="${Number(м_Настройки.Получить('лМенятьГромкостьКолесом'))}"]`))
		{
			уз.checked = true;
		}
		...
		{
			_оНачалоВоспроизведения     = new ВводЧисла(МИН_РАЗМЕР_БУФЕРА,               МАКС_РАЗМЕР_БУФЕРА,        0.5, 1, '',      'чНачалоВоспроизведения', 'началовоспроизведения');
			_оРазмерБуфера              = new ВводЧисла(МИН_РАЗМЕР_БУФЕРА,               МАКС_РАЗМЕР_БУФЕРА,        0.5, 1, '',      'чРазмерБуфера',          'размербуфера');
			_оРастягиваниеБуфера        = new ВводЧисла(МИН_РАСТЯГИВАНИЕ_БУФЕРА,         МАКС_РАСТЯГИВАНИЕ_БУФЕРА,  0.5, 1, '',      'чРастягиваниеБуфера',    'растягиваниебуфера');
			_оИнтервалОбновленияСписков = new ВводЧисла(МИН_ИНТЕРВАЛ_ОБНОВЛЕНИЯ_СПИСКОВ, 250,                       10,  0, 'J0120', 'чИнтервалОпроса',        'интервалопроса');
			_оДлительностьПовтора       = new ВводЧисла(МИН_ДЛИТЕЛЬНОСТЬ_ПОВТОРА,        МАКС_ДЛИТЕЛЬНОСТЬ_ПОВТОРА, 30,  0, 'J0124', 'чДлительностьПовтора2',  'длительностьповтора');
			_оНачалоВоспроизведения.ПослеИзменения = _оРазмерБуфера.ПослеИзменения =
			_оРастягиваниеБуфера.ПослеИзменения = _оИнтервалОбновленияСписков.ПослеИзменения = м_Статистика.ОчиститьИсторию;
		}
	}
	...
	function ОбработатьИзменениеСкоростиВоспроизведения(оСобытие)
	{
		try
		{
			if (_чСостояние === СОСТОЯНИЕ_ПОВТОР)
	...
	function ОбработатьИзменениеВариантаТрансляции(оСобытие)
	{
		try
		{
	...
	function Запустить()
	{
		...
		{
			уз.text = м_и17я.ФорматироватьЧисло(уз.value, 2);
		}
		...
		document.getElementById('проигрывательичат').addEventListener('click', ОбработатьЩелчок);
		уз = document.getElementById('глаз');
	...
	function ИзменитьСостояние(чНовоеСостояние)
	{
		...
		case СОСТОЯНИЕ_ПОВТОР:
			...
			ПоказатьМетаданныеТрансляции({кЗрителей: null});
			м_Помойка.Сжечь();
			break;
	...
	function ПоказатьМетаданныеКанала(оМетаданные)
	{
		...
		{
			ИзменитьЗаголовокДокумента(оМетаданные.сИмя + Текст('J0105'));
		...
		if (оМетаданные.сКодЯзыка !== undefined)
		{
			...
			if (оМетаданные.сКодЯзыка)
			{
				...
				уз.parentNode.removeAttribute('hidden');
			}
			else
			{
				уз.parentNode.setAttribute('hidden', '');
			}
		}

		if (оМетаданные.кПодписчиков !== undefined)
		{
			document.getElementById('канал-подписчиков').textContent = м_и17я.ФорматироватьЧисло(оМетаданные.кПодписчиков);
		}

		if (оМетаданные.кПросмотров !== undefined)
		{
			document.getElementById('канал-просмотров').textContent = м_и17я.ФорматироватьЧисло(оМетаданные.кПросмотров);
		}
	...
	{
		if (оМетаданные.лАвторизован !== undefined)
		{
			document.body.classList.toggle('авторизован', оМетаданные.лАвторизован);
		}

		if (оМетаданные.сИмя !== undefined)
		{
			Проверить(оМетаданные.сИмя);
		...
		if (оМетаданные.чПодписка !== undefined)
		{
			...
			{
				if (уз.classList.contains('обновляется'))
				{
					return false;
				}
	...
	// undefined - не менять, null - скрыть.
	{
		...
		if (оМетаданные.кЗрителей !== undefined)
		{
			...
			{
				уз.lastElementChild.textContent = м_и17я.ФорматироватьЧисло(оМетаданные.кЗрителей, 0);
				уз.removeAttribute('hidden');
			}
		}
		...
		if (оМетаданные.чДлительностьТрансляции !== undefined)
		{
			document.getElementById('позиция-текст').textContent =
				...
				: м_и17я.ПеревестиСекундыВСтроку(оМетаданные.чДлительностьТрансляции / 1000, false);
		}
	}
	...
	return {
		...
		ПоказатьМетаданныеКанала, ПоказатьМетаданныеЗрителя, ПоказатьМетаданныеТрансляции,
		ОтключитьПолноэкранныйРежим,
		ПереключитьПросмотрТрансляции
	};
})();

const м_Проигрыватель = (function()
{
	...
	var _oMediaElement;
	var _oMediaSource;
	var _oMediaSourceBuffer = null;
	// 0: начальное значение.
	// 1: _oMediaSource открывался.
	// 2: воспроизведение начиналось.
	var _чВоспроизведениеНачиналось = 0;
	var _лАсинхроннаяОперация = false;
	var _сРазмерБуфера = 'чНачалоВоспроизведения';
	var _лЖдатьЗаполненияБуфера = true;
	var _чСмещениеНачалаТрансляции = 0;
	...
	var _лНужнаПеремотка = false;


	var _оПрямаяТрансляция =
	{
	...
	var _оПоведение = _оПрямаяТрансляция;

	function ПоказатьСостояние(сВажность, сЗапись)
	{
		var оБуфер = _oMediaSource.sourceBuffers.length !== 0 ? _oMediaSource.sourceBuffers[0] : null;
		var сОбластиБуфера = ПеревестиОбластиВСтроку(оБуфер ? оБуфер.buffered : null);
		var сОбласти = ПеревестиОбластиВСтроку(_oMediaElement.buffered)
		var лОбластиРавны = сОбластиБуфера === сОбласти;
		...
		if (м_Статистика.ОкноПоказано())
		{
			...
			if (оБуфер.length !== 0)
			{
				document.getElementById('статистика-продолжительностьтрансляции').textContent =
					м_и17я.ПеревестиСекундыВСтроку(оБуфер.end(оБуфер.length - 1) + _чСмещениеНачалаТрансляции, true);
			}
		}
	}
	...
	function СледитьЗаСобытиямиMediaSource(оСобытие)
	{
		try
		{
			СледитьЗаОшибками();
	...
	function СледитьЗаСобытиямиMediaElement(оСобытие)
	{
		try
		{
			СледитьЗаОшибками();
	...
	function ДобавитьСледующийСегмент()
	{
		...
		var сГотовность = _oMediaSource.readyState;
		if (сГотовность !== 'open')
		{
		...
		if (оСегмент.пДанные.мбСегментИнициализации)
		{
			...
			{
				return ДобавитьСегмент(оСегмент, true);
			});
		}
		...
		.then(function()
		{
			...
			if (!(г_моОчередь[0] && г_моОчередь[0].пДанные === СОСТОЯНИЕ_ПОВТОР))
			{
				...
				var чПроверка = ПроверитьПозициюВоспроизведения(ПРОВЕРИТЬ_ДОБАВЛЕНИЕ_СЕГМЕНТА);
...
const м_Список = (function()
{
		...
		if (лЭтоСписокВариантов)
		{
			...
			for (var оВариант of моСписокВариантов)
			{
				...
				else
				{
					...
					оВариант.сНазвание = `${м_и17я.ФорматироватьЧисло(оВариант.nPeakBitrate / 1000000, 1)} ${Текст('J0114')}`;
				}
	...
	// Изменяет _чСостояние и _оПоследнийСписок.
	{
		if (_оПоследнийСписок === null)
		{
			...
			var чПервыйДобавленныйСегмент = 0;
		...
		else
		{
			...
			var кСегментовУдалено = оНовыйСписок.nMediaSequenceNumber - _оПоследнийСписок.nMediaSequenceNumber;
			if (кСегментовУдалено < 0)
			{
				// Twitch, ТВЦ: По ошибке нам прислали старый список. Выкидываем его и ждем следующий.
				// TODO ТВЦ: Пропускать копии сегментов когда новый nMediaSequenceNumber сравняется со старым.
				м_Журнал.Ой(`[Список] Новый #EXT-X-MEDIA-SEQUENCE на ${-кСегментовУдалено} меньше последнего`);
				return 0;
			}
			...
			чПервыйДобавленныйСегмент = _оПоследнийСписок.моСегменты.length - кСегментовУдалено;
			чИндекс = 0;

			if (чПервыйДобавленныйСегмент < 0)
			{
				// Firefox 43: Из-за нехватки памяти в системе, загрузка списка может задержаться на несколько десятков секунд.
				м_Журнал.Ой(`[Список] Пропущено сегментов: ${-чПервыйДобавленныйСегмент}`);
				м_Статистика.НеЗагруженыСегменты(-чПервыйДобавленныйСегмент);
				_лРазрыв = true;
			}

			if (оНовыйСписок.nTargetDuration !== _оПоследнийСписок.nTargetDuration)
			{
				м_Журнал.Ой(`[Список] TargetDuration изменился с ${_оПоследнийСписок.nTargetDuration} на ${оНовыйСписок.nTargetDuration}`);
				СброситьИнтервалОбновленияСписков();
			}
		}
		...
		{
			if (чИндекс < чПервыйДобавленныйСегмент)
			{
				// HACK Twitch: EXT-X-MEDIA-SEQUENCE на 1 меньше после начала трансляции.
				if (_оПоследнийСписок.моСегменты[кСегментовУдалено + чИндекс].сАдрес !== оСегмент.сАдрес
				||  _оПоследнийСписок.моСегменты[кСегментовУдалено + чИндекс].чДлительность !== оСегмент.чДлительность)
				{
					м_Журнал.Ой(`[Список] У сегмента ${оНовыйСписок.nMediaSequenceNumber + чИндекс} изменился адрес ${_оПоследнийСписок.моСегменты[кСегментовУдалено + чИндекс].сАдрес} ==> ${оСегмент.сАдрес} или длительность ${_оПоследнийСписок.моСегменты[кСегментовУдалено + чИндекс].чДлительность} ==> ${оСегмент.чДлительность}`);
					_лРазрыв = true;
				}
			}
			else
			{
				var оДобавлено = г_моОчередь.Добавить(new Сегмент(ОБРАБОТКА_ЖДЕТ_ЗАГРУЗКИ, оСегмент.сАдрес, оСегмент.чДлительность, оСегмент.лРазрыв || _лРазрыв, оСегмент.чTwitchПрошлоВремени));
				м_Журнал.Вот(`[Список] Добавлен сегмент ${оДобавлено.чНомер} MediaSequenceNumber=${оНовыйСписок.nMediaSequenceNumber + чИндекс} Длительность=${оДобавлено.чДлительность} Разрыв=${оДобавлено.лРазрыв} TwitchПрошлоВремени=${оСегмент.чTwitchПрошлоВремени}`);
				_лРазрыв = false;
...
const м_Преобразователь = (function()
{
	...
	// Версии player.js и worker.js должны совпадать. Для этого, после обновления расширения, необходимо страницу перезагрузить
	// или закрыть. В Firefox расширение перезагружает все страницы с проигрывателем. Chrome сам закрывает все страницы с
	// проигрывателем (зрителям наверное это не очень нравится).
	{
		if (!_оРабочийПоток)
		{
...
const м_Загрузчик = (function()
{
	...
	// BUG Firefox 46, Chrome 48, Edge 15 (выход из спячки): Иногда запрос не завершается, не посылая никаких событий.
	{
		...
		return new Promise(function(фВыполнить, фОтказаться)
		{
			...
			try
			{
				// HACK Firefox 49- может кинуть исключение.
				// https://www.fxsitecompat.com/en-CA/docs/2016/network-error-for-async-xhr-now-fires-error-event-instead-of-throwing-getallresponseheaders-will-be-empty/
				оЗапрос.send();
			}
			catch (пИсключение)
			{
				ОбработатьОшибку({type: String(пИсключение)});
			}
			...
			function ОбработатьLoad()
			{
				try
				{
					...
					else
					{
						...
						Проверить(чКод >= 400 && чКод <= 599);
						if (чКод < 500 || чКод > 599 || !ПовторитьЗапрос())
						{
...
{
	const ИНТЕРВАЛ_ОБНОВЛЕНИЯ_МЕТАДАННЫХ = 60000; // Миллисекунды.
	...
	var _сИдЗрителя    = '';
	var _сТокенЗрителя = '';
	var _ооОбновлениеМетаданных = new ОтменаОбещания();
	...
	function ПолучитьИлиИзменитьПодпискуЗрителяНаКанал(чПодписка)
	{
		...
		if (_сИдЗрителя === _сИдКанала)
		{
			...
			м_Управление.ПоказатьМетаданныеЗрителя({чПодписка: ПОДПИСКА_НЕДОСТУПНА});
			return;
		}
		switch (чПодписка)
		{
		...
		case ПОДПИСКА_НЕУВЕДОМЛЯТЬ:
			var сМетод = 'PUT';
			var сПараметры = '?notifications=false';
			break;
		case ПОДПИСКА_УВЕДОМЛЯТЬ:
			var сМетод = 'PUT';
			var сПараметры = '?notifications=true';
			break;
		default:
			Проверить(false);
		}
		...
		.then(function(сРезультат)
		{
			...
			if (чПодписка === ПОДПИСКА_НЕОФОРМЛЕНА)
			{
				...
				м_Управление.ПоказатьМетаданныеЗрителя({чПодписка: ПОДПИСКА_НЕОФОРМЛЕНА});
				return;
			}
			try
			{
				var оРезультат = JSON.parse(сРезультат);
			}
			catch (пИсключение)
			{
			...
			м_Управление.ПоказатьМетаданныеЗрителя(
			{
	...
	function ПолученИдКанала()
	{
		try
		{
			ПолучитьМетаданныеКанала();
			ПолучитьМетаданныеЗрителя(true);
		}
		catch (пИсключение)
		{
			м_Отладка.ПойманоИсключение(пИсключение);
			...
			function(сРезультат)
			{
				...
				if (ЭтоОбъект(оРезультат) && ЭтоНепустаяСтрока(оРезультат.token) && ЭтоНепустаяСтрока(оРезультат.sig))
				{
					...
					if (оТокен.channel === _сКодКанала)
					{
						...
						{
							// TODO Теоретически проверка может сработать, если во время трансляции канал поменяет код, а другой канал
							// возьмет себе этот код, и метаданные трансляции не успеют получить новый код?
	...
	function ПолучитьМетаданныеКанала()
	{
		...
		.then(function(сРезультат)
		{
			...
			м_Управление.ПоказатьМетаданныеКанала(
			{
				сИмя: оРезультат.display_name || оРезультат.name,
				сАватар: оРезультат.logo || chrome.extension.getURL('glitch.svg'),
		...
		.then(function(сРезультат)
		{
			...
			for (var оСообщество of оРезультат.communities)
			{
			...
			м_Управление.ПоказатьМетаданныеКанала({моСообщества: оРезультат.communities});
		})
		.catch(function(пПричина)
		{
			if (typeof пПричина === 'string')
		...
		.then(function(сРезультат)
		{
			...
			for (var оКоманда of оРезультат.teams)
			{
			...
			м_Управление.ПоказатьМетаданныеКанала({моКоманды: оРезультат.teams});
		})
		.catch(function(пПричина)
		{
			if (typeof пПричина === 'string')
	...
	function ПолучитьМетаданныеЗрителя()
	{
		...
		.then(function(сРезультат)
		{
			...
			м_Управление.ПоказатьМетаданныеЗрителя({сИмя: оРезультат.display_name || оРезультат.name});
		...
		.then(function(сРезультат)
		{
			...
			м_Управление.ПоказатьМетаданныеЗрителя({лАвторизован: true});
		...
		.catch(function(пПричина)
		{
			try
			{
				if (typeof пПричина === 'string')
				{
					...
					м_Управление.ПоказатьМетаданныеЗрителя({лАвторизован: false});
				}
				// Отмены обещания в этой цепочке пока нет.
				else
				{
	...
	function ОбновитьМетаданныеТрансляции(чЧерез)
	{
		...
		.then(function(сРезультат)
		{
			...
			// live - прямая трансляция, watch_party - vodcast.
			const лЗапись = оРезультат.stream.stream_type !== 'live';
			const чНачалоТрансляции = Date.parse(оРезультат.stream.created_at); // ISO 8601
			Проверить(
				...
				&& Number.isFinite(чНачалоТрансляции)
			);
			м_Управление.ПоказатьМетаданныеТрансляции(
			{
				...
				чДлительностьТрансляции: Date.now() - чНачалоТрансляции
			});
			...
			// Теоретически возможна ситуация, когда сервер начал другую трансляцию, а старая еще не досмотрена (часть находится в буфере проигрывателя).
			if (_сИдТрансляции === '')
			{
				...
				if (лЗапись)
				{
					м_Управление.ПоказатьМетаданныеТрансляции({лЗапись: true, сАдресЗаписи: ''});
				}
				else
				{
					м_Управление.ПоказатьМетаданныеТрансляции({лЗапись: false, сАдресЗаписи: ''});
					...
					.then(function(сРезультат)
					{
						...
						for (var оЗапись of оРезультат.videos)
						{
							if (String(оЗапись.broadcast_id) === _сИдТрансляции)
							{
								Проверить(оЗапись.broadcast_type === 'archive');
								Проверить(String(оЗапись.channel._id) === _сИдКанала);
								м_Управление.ПоказатьМетаданныеТрансляции({лЗапись: false, сАдресЗаписи: оЗапись.url});
								break;
							}
						}
					})
					.catch(function(пПричина)
					{
						// Ошибки в этой цепочке не должны влиять на основную цепочку и укорачивать ИНТЕРВАЛ_ОБНОВЛЕНИЯ_МЕТАДАННЫХ.
						if (typeof пПричина === 'string')
						{
		...
		.then(function()
		{
			ОбновитьМетаданныеТрансляции(ИНТЕРВАЛ_ОБНОВЛЕНИЯ_МЕТАДАННЫХ);
		})
		.catch(function(пПричина)
		{
			try
			{
				...
				if (typeof пПричина === 'string')
				{
					...
					ОбновитьМетаданныеТрансляции(ИНТЕРВАЛ_ОБНОВЛЕНИЯ_МЕТАДАННЫХ / 2);
				}
				// Загрузка отменена?
				else if (пПричина === ОтменаОбещания.ОБЕЩАНИЕ_ОТМЕНЕНО)
				{
	...
	{
		var чПозиция = Math.round(м_Проигрыватель.ПолучитьПозициюВоспроизведенияТрансляции() - 3);
		...
		{
			м_Журнал.Ой(`[Twitch] Недостаточно данных для создания клипа ИдТрансляции=${_сИдТрансляции} Позиция=${чПозиция} ИдЗрителя=${_сИдЗрителя}`);
		}
	}
	...
	return {
		ПолучитьАбсолютныйАдресСпискаВариантов, ПолучитьАдресУбогогоПроигрывателя, ПолучитьАдресПанелиЧата,
...
function ПроизвольныйСписокВариантов(_сАбсолютныйАдресСпискаВариантов)
{
	...
	return {
		ПолучитьАбсолютныйАдресСпискаВариантов, ПолучитьАдресУбогогоПроигрывателя: НетАдреса, ПолучитьАдресПанелиЧата: НетАдреса,
...
function ВыделитьКанал(оАдрес)
{
	...
	var сКанал = РазобратьПараметры(оАдрес).get('channel') || 'channel';
...
function ЭтотКаналУжеОткрыт(сКодКанала)
{
	...
	for (var оОкно of chrome.extension.getViews())
	{
		{
			if (оОкно === window || оОкно.closed || ВыделитьКанал(оОкно.location).сКодКанала !== сКодКанала)
			{
				continue;
			}
		}
		оОкно.chrome.tabs.getCurrent(function(оВкладка)
		{
			try
			{
				if (!оОкно.chrome.runtime.lastError)
				{
...
function ЗавершитьРаботу(лБыстро)
{
	...
	try
	{
		...
		м_Twitch.ЗавершитьСборМетаданныхТрансляции();
		м_Список.Остановить();
		м_Преобразователь.Остановить();
		// Firefox 47: Если после beforeunload начать загрузку файла, то это замедлит перезагрузку страницы.
		г_моОчередь.Очистить();
		м_Чат.ЗакрытьОкно();
...
(function()
{
	try
	{
		...
		Promise.all([м_Настройки.Восстановить(), м_Оформление.ЗапуститьАсинхронно(), ЖдатьЗагрузкуСтраницы()])
		.then(НачатьРаботу)
