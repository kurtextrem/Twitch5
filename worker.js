...
{
	// HACK Firefox 49: Не выбрасывать буфер, который используется в AsmJS в качестве кучи, чтобы
	// не вызвать исключение out of memory. К счастью, выделять буфер для AsmJS приходится редко.
	if (мбБарахло && мбБарахло.buffer.byteLength && typeof мбБарахло.НайтиПрефикс === 'undefined')
	{
...
function AsmJS(оБиблиотека, оИмпорт, буфКуча)
...
// нет полноценного AOT-компилятора asm.js.
{
	...
	function SearchStartCodePrefix(уНачало, уКонец)
	...
	// Если префикс не найден, возвращает уКонец.
	// Если ошибка в параметрах, возвращает -1.
	// Если данные повреждены, возвращает -2.
	// уНачало и уКонец не должны выходить за пределы массива.
	// уНачало и уКонец не должны превышать 0x7fffffff.
	...
	// но это разница может меняться в разных версиях браузеров.
	{
		...
		var уКонец3 = 0, чБайт = 0, уНачалоПрефикса = 0;

		if (((уНачало|0) < 0) | ((уНачало|0) > (уКонец|0)))
		{
			return -1|0;
		}
		...
		for (;;)
		{
			if ((уНачало|0) > (уКонец3|0))
			{
				return уКонец|0;
			}
			чБайт = _мбКуча[уНачало >> 0]|0;
			уНачало = (уНачало + 1)|0;
			if ((чБайт|0) == 0)
			{
				чБайт = _мбКуча[уНачало >> 0]|0;
		...
		do
		{
			...
			if ((чБайт|0) == 1)
			{
				уНачало = (уНачало - уНачалоПрефикса)|0;
				// TODO При необходимости увеличить размер до 3-х байт.
				if ((уНачало|0) > 0xffff)
				{
					return -1|0;
				}
				_мбКуча[уНачалоПрефикса >> 0] = уНачало;
				_мбКуча[(уНачалоПрефикса + 1) >> 0] = уНачало >> 8;
				return уНачалоПрефикса|0;
			}
			if ((чБайт|0) != 0)
			{
...
(function()
{
	...
	***/
	...
	{
		// HACK РАЗМЕР_ТРАНСПОРТНОГО_ПАКЕТА нужен для Firefox в РазобратьТранспортныйПоток().
		var кбРазмер = Math.ceil(кбЗаполнен + кбДолжноБытьСвободно + РАЗМЕР_ТРАНСПОРТНОГО_ПАКЕТА);
	...
	function ВыделитьПамять(кбРазмерСегмента, чДлительностьСегмента)
	{
		...
		_мбВидеоПоток  = ВыделитьМассив(_мбВидеоПоток,  _срЗадержанноеВидео.уКонецПотока,  кбВидеоПоток,  ОПЕРЕЖАЮЩЕЕ_УВЕЛИЧЕНИЕ_ВИДЕО,   true);
	...
	function РазобратьТранспортныйПоток(мбТранспортныйПоток)
	{
		...
		var мб1йТранспортныйПакет = new Uint8Array(мбТранспортныйПоток.buffer, 0, РАЗМЕР_ТРАНСПОРТНОГО_ПАКЕТА - 4); // Максимальный размер payload.
			...
			уТранспортныйПакет += РАЗМЕР_ТРАНСПОРТНОГО_ПАКЕТА)
		{
			...
			// HACK Firefox 54: new Uint8Array() занимает много времени, поэтому РазобратьТранспортныйПоток() в 4 раза медленнее,
			// чем в Chrome. copyWithin() с использованием одного заранее созданного Uint8Array ускоряет РазобратьТранспортныйПоток()
			// в 3 раза, несмотря на лишнее копирование. Размер мбПоток нужно дополнительно увеличить на РАЗМЕР_ТРАНСПОРТНОГО_ПАКЕТА.
			мбТранспортныйПоток.copyWithin(0, pPayload, pPayload + cbPayload);
			мбПоток.set(мб1йТранспортныйПакет, срОбработать.уКонецПотока);
