...
function ЗапроситьСостояниеКанала(оРазобранныйАдрес)
{
	...
	if (!оРазобранныйАдрес.лМожноПеренаправлять || !АвтоперенаправлениеВключено())
	{
		return;
	}
	// Ответ уже получен?
...
function ИзмененАдресСтраницы(сИсторияВкладки)
{
	...
	// Адрес перенаправлять не нужно?
	if (!г_оРазобранныйАдрес.лМожноПеренаправлять || !АвтоперенаправлениеВключено())
	{
...
{
	var с = localStorage.getItem('Twitch5Перенаправлять');
	return с === null || с !== 'false';
}

function ОбновитьКнопкуПереключенияАвтоперенаправления()
{
	ОбновитьСостояниеЭлемента(document.getElementsByClassName('tw5-автоперенаправление')[0], АвтоперенаправлениеВключено());
}
...
{
	оСобытие.preventDefault();
	const сАвтоперенаправление = АвтоперенаправлениеВключено() ? 'false' : 'true';
	Журнал('[Twitch 5] Автоперенаправление:', сАвтоперенаправление);
	localStorage.setItem('Twitch5Перенаправлять', сАвтоперенаправление);
	ОбновитьКнопкуПереключенияАвтоперенаправления();
}

function ВставитьКнопкуПереключенияАвтоперенаправления() // UNDONE Переименовать.
...
function СледитьЗаРазметкойСтраницы()
{
	...
	{
		if (ВставитьКнопкуПереключенияАвтоперенаправления())
		{
